...
[Serializable]
public class WhisperModelConfig
{
    public string name; // Friendly name cho UI
    public string modelPath;
    public bool isInStreamingAssets = true;
}
...
namespace Whisper
{
    ...
    public class WhisperManager : MonoBehaviour
    {
        ...
        [Header("Available Whisper Models")]
        public List<WhisperModelConfig> availableModels = new List<WhisperModelConfig>();

        [Header("Model Selection")]
        [Tooltip("Index of the model to use from availableModels")]
        ...
        {
            // Nếu model đã load, giải phóng trước
            if (IsLoaded)
            {
                UnloadModel();
            }
            ...
            try
            {
                ...
                if (availableModels != null && availableModels.Count > 0 && currentModelIndex >= 0 && currentModelIndex < availableModels.Count)
                {
                    var selectedModel = availableModels[currentModelIndex];
                    modelPath = selectedModel.modelPath;
                    isModelPathInStreamingAssets = selectedModel.isInStreamingAssets;
                    Debug.Log($"[WhisperManager] Current model: {selectedModel.name} | Path: {modelPath}");
                }
        ...
        public void UnloadModel()
        {
            if (_whisper != null)
            {
                _whisper.Dispose(); // hoặc hàm giải phóng tương ứng nếu có
                _whisper = null;
                _params = null;
                IsLoading = false;
                Debug.Log("[WhisperManager] Model unloaded.");
            }
        }
    }
}